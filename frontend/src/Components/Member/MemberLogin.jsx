...
import LoadingPage from "../Website/Loading"

function MemberLogin() {

  const auth = getAuth();
  const { user, member, SetUser, notify, fetchMemberDetails } = useContext(MainContext)
  ...
  const LoginUserHandler = (e) => {
    ...
    signInWithEmailAndPassword(auth, data.email, data.password)
      .then((userCredential) => {
        const isMember = userCredential.user;
        e.target.reset()
        SetLoading(false)
        if (isMember) {
          fetchMemberDetails(isMember.uid)
      ...
      <div className="w-full max-w-[500px] bg-[#1C1C1C] p-8 rounded-lg shadow-lg">
        ...
        <div className="text-center mt-4">
          <p className="text-gray-400 text-sm">
            👤 Not a member? Login as a User
            <Link to="/login" className="text-[#CAFF33] hover:underline">
              [Click here]
            </Link>
          </p>
        </div>
        ...
        <div className="text-center mt-6">
          <p className="text-gray-400 text-sm">
            ...
            <Link to="/signup" className="text-[#CAFF33] hover:underline">
              Click here
            </Link>
          </p>
        </div>
